...
class ResponseHydrator
{
    ...
    public function hydrate($request, $rawResponse)
    {
        ...
        if ($rawResponse) {
            ...
            $responseData = array();
            if ($request->getEncapChar()) {
                $responseData = explode($request->getEncapChar()
                    . $request->getDelimChar()
                    . $request->getEncapChar(), substr($rawResponse, 1, -1));
            } else {
                $responseData = explode($request->getDelimChar(), $rawResponse);
            }
